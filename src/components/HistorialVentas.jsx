        ...
        <div className="overflow-y-auto bg-white text-black w-full sm:w-[90%] h-[70%]">
          {history.length > 0 ? (
            <table className="table-auto text-xl w-full text-center border">
              <thead>
                <tr>
                  <td className="border-2 font border-black bg-amber-200 py-2">Fecha y Hora</td>
                  <td className="border-2 border-black bg-red-200 py-2">Nombre</td>
                  <td className="border-2 border-black bg-green-200 py-2">Cantidad</td>
                  <td className="border-2 border-black bg-cyan-200 py-2">Precio Unitario</td>
                  <td className="border-2 border-black bg-indigo-200 py-2">Precio Total</td>
                </tr>
              </thead>
              <tbody>
                {history.map((venta, index) => (
                  <>
                    <tr>
                      <td colSpan={5} className="border-2 px-4 py-2 border-black">
                        Venta {index + 1}
                      </td>
                    </tr>
                    {venta.map((item) => {
                      return (
                        <tr key={item.FechaYHora}>
                          <td className="border border-black py-2">{item.FechaYHora}</td>
                          <td className="border border-black py-2">{item.name}</td>
                          <td className="border border-black py-2">{item.cantidad}</td>
                          <td className="border border-black py-2">${item.price}</td>
                          <td className="border border-black py-2">${item.precioTotal}</td>
                        </tr>
                      );
                    })}
                    <tr>
                      <td colSpan={3}></td>
                      <td className="border-2 border-black px-4 py-2">Total Venta</td>
                      <td className="border-2 border-black px-4 py-2">
                        ${venta.reduce((acc, curr) => acc + curr.price, 0)}
                      </td>
                    </tr>
                  </>
                ))}
